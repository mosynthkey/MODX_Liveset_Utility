...
class ModxLivesetUtility {
    ...
    setX8ADataBuffer(x8aDataBuffer) {
        console.log(Object.prototype.toString.call(x8aDataBuffer));
...
function printLivesetContents(fileName) {
    try {
        ...
        utility.setX8ADataBuffer(buffer);
...
const indexOfArray = (fromArray, searchArray) => {
    let fromIndex = 0;
    while (fromIndex + searchArray.length < fromArray.length) {
        let index = fromArray.indexOf(searchArray[0], fromIndex);
        if (index == -1) return -1;
        const subarray = fromArray.subarray(index, index + searchArray.length);
        if (subarray.toString() == searchArray.toString()) return index;
        fromIndex = index + 1;
    }
};
